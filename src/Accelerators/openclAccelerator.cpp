...
void OpenclAccelerator::do_forwardpass_to_current_layer(uint unInLayerIdx)
{
...
void OpenclAccelerator::find_delta_of_current_layer_nodes(float* pfExpOut, uint i)
{
    //i = layer index
    ...
    else //hidden layer
    {
        ...
        const float * wtMtx = m_pNN->GetMatrix(i)->getWtMtx();
        uint out_lyr_sz = m_pNN->GetLayer(i+1)->get_num_nodes();
        uint in_lyr_sz = m_pNN->GetLayer(i)->get_num_nodes();
        float* outLyrNodeDelta = new float[out_lyr_sz];
        // float * wtNodeProd = new float [in_lyr_sz * out_lyr_sz];
        for(k = 0; k < out_lyr_sz; k++)
        {
            outLyrNodeDelta[k] = m_pNN->GetLayer(i+1)->get_node_delta_idx(k);
        }
